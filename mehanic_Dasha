...
class Game:
    ...
    def draw_pixel_grass(self):
        base_colors = [(90,160,50), (70,140,40), (100,180,60), (80,150,45)]
        size = 4
        for y in range(0, GameSettings.SCREEN_HEIGHT, size):
            for x in range(0, GameSettings.SCREEN_WIDTH, size):
                color = base_colors[(x//size + y//size) % len(base_colors)]
                self.screen.fill(color, rect=(x, y, size, size))

        # Рисуем травинки — тонкие линии случайной длины и цвета
        for _ in range(400):
            x = random.randint(0, GameSettings.SCREEN_WIDTH - 1)
            y = random.randint(0, GameSettings.SCREEN_HEIGHT - 1)
            length = random.randint(3, 7)
            green_shade = base_colors[random.randint(0, len(base_colors) - 1)]
            pygame.draw.line(self.screen, green_shade, (x, y), (x, y - length))

    def draw(self):
        if not self.screen:
            return

        # Заменяем фон на пиксельную траву
        self.draw_pixel_grass()

        # Отрисовка дорог (перекрёстков и машин)
        for crossroad in self.crossroads:
        ...
        # Отрисовка препятствий (деревья, дома)
        for obstacle in self.obstacles:
